...
final class GitFixture: Sendable {
    ...
    init() async throws {
        root = FileManager.default.temporaryDirectory.appendingPathComponent("fetchbar-tests-\(UUID().uuidString)", isDirectory: true)
        ...
        guard let found = await locator.locate(override: ProcessInfo.processInfo.environment["FETCHBAR_TEST_GIT"].map { URL(fileURLWithPath: $0) }) else {
