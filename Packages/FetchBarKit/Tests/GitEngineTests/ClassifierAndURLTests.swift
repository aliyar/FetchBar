...
struct RemoteURLTests {
    ...
    /// A page FetchBar cannot address is left out instead of guessed at: the menu is built
...
struct GitLocatorTests {
    ...
    @Test func locatesFakeGitScriptFirst() async throws {
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("fetchbar-locator-\(UUID().uuidString)")
