...
struct ProcessRunnerTests {
    ...
    @Test func cancellationBeforeLaunchThrowsWithoutRunning() async throws {
        let runner = self.runner
        let marker = FileManager.default.temporaryDirectory.appendingPathComponent("fetchbar-\(UUID().uuidString)")
    ...
    @Test func workingDirectoryAndEnvironmentAreApplied() async throws {
        ...
        var s = spec("pwd; printf %s \"$FETCHBAR_TEST\"")
        s.workingDirectory = dir
        s.environment["FETCHBAR_TEST"] = "yes"
        let result = try await runner.run(s)
