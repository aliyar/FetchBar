...
TODAY=$(date +%Y-%m-%d)
URL="https://github.com/$REPO/releases/download/v$VERSION/FetchBar-$VERSION.zip"
...
edit "s|releases/download/v[0-9][0-9.]*/FetchBar-[0-9][0-9.]*\.zip|releases/download/v$VERSION/FetchBar-$VERSION.zip|g" \
     "$URL" "download links"
edit "s|Download FetchBar [0-9][0-9.]*<|Download FetchBar $VERSION<|" \
     "Download FetchBar $VERSION<" "download button label"
...
edit "s|<span class=\"mono\">[0-9a-f]\{64\}</span>|<span class=\"mono\">$SHA</span>|" \
     "$SHA" "SHA-256"
edit "s|FetchBar <span class=\"mono\">[0-9][0-9.]*</span>|FetchBar <span class=\"mono\">$VERSION</span>|" \
     "FetchBar <span class=\"mono\">$VERSION</span>" "footer version"
...
STALE=$(grep -oE 'FetchBar-[0-9]+\.[0-9]+\.[0-9]+\.zip' "$PAGE" | grep -v "FetchBar-$VERSION.zip" || true)
